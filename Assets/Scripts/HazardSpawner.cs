...
{
    [SerializeField] private PoolAbleObject m_Meteor;
    [SerializeField] private PoolAbleObject m_Alien;
    [SerializeField] private PoolAbleObject m_HealingAlien;
    ...
    private void Start()
    {
        ...
        {
            Debug.LogError("No transforms have been added to the list!");
        }
        else
        {
            m_FirstSpawn = true;
    ...
    private void SpawnItem(Transform spawnPos)
    {
        ...
        {
            GameObject alien = PoolManager.Instance.GetObjectFromPool(m_Alien);
            //ObjectPooler.m_Instance.SetActiveFromPool(WhichPrefab.anvil, spawnPos.position, Quaternion.identity);
        }
        else if (randomNumber > 95)// && GameManager.Instance.GetCurrentPlayer().GetIfPlayerIsHurt() && !m_FirstSpawn)
        {
            GameObject healingAlien = PoolManager.Instance.GetObjectFromPool(m_HealingAlien);
            //ObjectPooler.m_Instance.SetActiveFromPool(WhichPrefab.heart, spawnPos.position, Quaternion.identity);
        }
        else
        {
            GameObject meteor = PoolManager.Instance.GetObjectFromPool(m_Meteor);
